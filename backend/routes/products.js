...
// Export as a function that accepts io instance
module.exports = function (io) {

    // Helper function to emit product status update
    const emitProductStatusUpdate = async (product, isActivated) => {
        try {
            if (isActivated) {
                // Product activated - emit to all clients
                io.emit('productActivated', product);
                io.emit('productStatusUpdate', { ...product.toObject(), isActive: true });
            } else {
                // Product deactivated - emit to all clients
                io.emit('productDeactivated', {
                    _id: product._id,
                    id: product._id,
                    name: product.name
                });
                io.emit('productStatusUpdate', { ...product.toObject(), isActive: false });
            }

            // Also send updated stock list
            const allProducts = await Product.find({ isActive: true });
            io.emit('stockUpdate', allProducts);

            console.log(`Socket emitted: product ${isActivated ? 'activated' : 'deactivated'} - ${product.name}`);
        } catch (error) {
            console.error('Error emitting product status update:', error);
        }
    };

    // Helper function to emit product update
    const emitProductUpdate = async (product) => {
        try {
            ...
            console.log(`Socket emitted: product updated - ${product.name}`);
        } catch (error) {
    ...
    router.post('/', adminAuth, async (req, res) => {
        try {
            ...
            emitProductStatusUpdate(product, true);
    ...
    router.put('/:id', adminAuth, async (req, res) => {
        try {
            ...
            if (isActive !== undefined && wasActive !== product.isActive) {
                emitProductStatusUpdate(product, product.isActive);
            } else {
    ...
    router.delete('/:id', adminAuth, async (req, res) => {
        try {
            ...
            // Emit socket event for deactivation
            emitProductStatusUpdate(product, false);
